...
export default class PlayRoute extends Route {
  ...
  async model() {
    ...
    // const transports = await this.store.findAll('transport');
    // const enemies = await this.store.findAll('enemy');
    return RSVP.hash({
      // data: data,
      player: this.store.findRecord('player', 1),
      weapons: this.store.findAll('weapon'),
      transports: this.store.findAll('transport'),
      enemies: this.store.findAll('enemy'),

      // TODO  ---> all the maps requests go somewhere else
